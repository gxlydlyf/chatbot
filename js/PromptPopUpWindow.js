window.PromptPopUpWindow = {
    ...
    messagePopUpWindow: function (config) {
        ...
        var isElement = this.isElement;
        var configuration = {
            confirmBtn: '<span class="PromptPopUpWindowButton">确定</span>'
        };
        configuration = this.mergeObjects(this.basicMinWindowConfig(), configuration)
        config = isObject(config) ? config : {};
        var getValHtml = function (key) {
            return This.getConfigValueOfHtml(config, configuration, key);
        }
        var getValStyle = function (key) {
            return This.getConfigValueOfStyle(config, configuration, key);
        }
        var getValFalse = function (key) {
            return This.getConfigValueOfFalse(config, configuration, key);
        }
        configuration.title = getValHtml('title');
        configuration.content = getValHtml('content');

        if (('callback' in config) && (typeof config.callback === 'function')) {
            configuration.callback = config.callback;
        }
        if (('mask' in config) && (typeof config.mask === 'boolean') && (config.mask === false)) {
            configuration.mask = config.mask;
        }
        var buttonGroup = $('<div class="PromptPopUpWindowButtonGroup"></div>');
        var confirmBtnElement;
        confirmBtnElement = $(getValFalse('confirmBtn'));
        if (('confirmBtn' in config) && (typeof config.confirmBtn === 'string')) {
            confirmBtnElement = $(configuration.confirmBtn).html(config.confirmBtn);
        }
        if (('confirmBtn' in config) && isElement(config.confirmBtn)) {
            confirmBtnElement = $(config.confirmBtn);
        }
        confirmBtnElement.on('click', function () {
            windowElement.data('close')();
        });
        ...
        if ('style' in config) {
            configuration.style.window = getValStyle('window');
        }
...
$(document)
    .on('mouseenter', '.PromptPopUpWindowButton', function () {
        // 鼠标进入时的处理逻辑
        $(this).animate({backgroundColor: '#007acb'}, 100);
        $(this).animate({color: '#f5f5f5'}, 100);
    })
    .on('mouseleave', '.PromptPopUpWindowButton', function () {
        // 鼠标离开时的处理逻辑
        $(this).stop().animate({backgroundColor: "#0385dc"}, 100);
